...
router.post('/', async (req, res) => {
  ...
  try {
    ...
    // 2. Buscar si existe como alumno en la escuela por nombre y apellido
    ...
    // 3. Si no existe como alumno → redirigir a matriculación
    ...
    // 5. Vincular el alumno con el nuevo usuario
    await pool.query(
      `UPDATE alumno SET id_usuario = $1 WHERE id_alumno = $2`,
      [usuario.id_usuario, alumno.rows[0].id_alumno]
    );

    // 6. Devolver el usuario para loguearlo directamente
    res.status(201).json({
